<?php
	class CdnWPFC{
		public static function cloudflare_clear_cache(){
			if($cdn_values = get_option("WpFastestCacheCDN")){
				...
				if($zone["success"]){
					...
					$response = wp_remote_request('https://api.cloudflare.com/client/v4/zones/'.$zone["zoneid"].'/purge_cache', $header);
				}
			}
		}
		...
		public static function cloudflare_get_zone_id($email = false, $key = false, $disable_minify = true){
			if(current_user_can('manage_options')){
				...
				if(!$response || is_wp_error($response)){
					...
					if(isset($zone->errors) && isset($zone->errors[0])){
						...
						if(isset($zone->result) && isset($zone->result[0])){
							$minify = CdnWPFC::cloudflare_disable_minify($email, $key, $zone->result[0]->id);
							$rocket_loader = CdnWPFC::cloudflare_disable_rocket_loader($email, $key, $zone->result[0]->id);
							...
							$purge_cache = wp_remote_request('https://api.cloudflare.com/client/v4/zones/'.$zone->result[0]->id.'/purge_cache', $header);
