...
if ( $qo instanceof WP_Post && $qo->ID !== 0 ) {
	$show_heading_title = kalium()->acf->get_field( 'heading_title', $qo->ID );
	
	if ( $show_heading_title ) {
		$title_type         = kalium()->acf->get_field( 'page_heading_title_type', $qo->ID );
		$description_type   = kalium()->acf->get_field( 'page_heading_description_type', $qo->ID );
		
		$custom_title       = kalium()->acf->get_field( 'page_heading_custom_title', $qo->ID );
		$custom_description = kalium()->acf->get_field( 'page_heading_custom_description', $qo->ID );
